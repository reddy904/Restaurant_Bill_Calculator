...
def enter():
	if name_inp.get() == "reddy" and password_inp.get() == "reddy12":
		root1.destroy()
		import question
	else:
...
label2 = Label(root1, font =('slant',10,'bold'),text="password")
...
enter_btn = Button(root1, font =('slant',10,'bold'),text="Enter", command= enter)
enter_btn.grid(row=3, column=1)

exit_btn = Button(root1, padx= 1,font =('slant',10,'bold'), text="Exit", command= destroy)
